...
async def change_role(user_id: uuid.UUID, body: RoleIn, request: Request, admin: AdminUser, db: DbSession):
    ...
    if user.role == "admin" and body.role == "user" and await _active_admin_count(db, exclude_id=user.id) == 0:
...
async def create_team(body: TeamCreateIn, request: Request, admin: AdminUser, db: DbSession):
    ...
    team = Team(name=name, slug=_slugify(name), is_default=False, created_by=admin.id)
    db.add(team)
    await db.flush()
