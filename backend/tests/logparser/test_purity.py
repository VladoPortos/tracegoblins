...
def test_logparser_imports_no_db_or_web():
    # Import the package in a FRESH interpreter (no conftest pre-loading sqlalchemy/
    ...
    # conftest already loaded, so we isolate in a subprocess.
    code = (
        "import sys\n"
    ...
    assert result.returncode == 0, (
        f"logparser must stay pure; leaked: {result.stdout.strip()}\n{result.stderr}"
    )
